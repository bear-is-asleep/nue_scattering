...
for i in range(1):#3
  for j in range(1):#2
    ...
    #Precut plots
    if i == 0:
      ax,fig = nueplotters.hist_scatback(nue_precut,back_precut,'genie_Eng',sb_precut,alpha=0.6,
                title=r'$E_e$ Distribution '+potstr+'\n'+'Precut',
                xlabel='Energy [GeV]',ylabel='',bw=bw_e)
      plotters.save_plot(f'E_pot{j}__precut',folder_name=energy_folder)
      ax,fig = nueplotters.hist_scatback(nue_precut,back_precut,'theta_t',sb_precut,alpha=0.6,
                title=r'$\theta_e$ Distribution '+potstr+'\nPrecut',
                xlabel=r'$\theta _e$ [rad]',ylabel='',bw=bw_t)
      ...
      plt.close()
      ax,fig = nueplotters.hist_scatback(nue_precut,back_precut,'E_theta^2',sb_precut,alpha=0.6,
    ...
    #Postcut plots
    ax,fig = nueplotters.hist_scatback(nue,back,'genie_Eng',sb,alpha=0.6,
              title=r'$E_e$ Distribution '+potstr+'\n'+r'$E_e \theta_e^2 <$'+f'{Etheta}',
              xlabel='Energy [GeV]',ylabel='',bw=bw_e)
    plotters.save_plot(f'E_{savestr}',folder_name=energy_folder)
    plt.close()
    ax,fig = nueplotters.hist_scatback(nue,back,'theta_t',sb,alpha=0.6,
              title=r'$\theta_e$ Distribution '+potstr+'\n'+r'$E_e \theta_e^2 <$'+f'{Etheta}',
              xlabel=r'$\theta _e$ [rad]',ylabel='',bw=bw_t)
    ax.set_xlim([-0.01,0.5])
    plotters.save_plot(f'thetae_{savestr}',folder_name=thetae_folder)
    plt.close()
    ax,fig = nueplotters.hist_scatback(nue,back,'E_theta^2',sb,alpha=0.6,
              title=r'$E_e \theta_e^2$ Distribution '+potstr+'\n'+r'$E_e \theta_e^2 <$'+f'{Etheta}',
              xlabel=r'$E_e \theta_e^2$ [GeV rad$^2$]',ylabel='',
              bw=bw_et)
